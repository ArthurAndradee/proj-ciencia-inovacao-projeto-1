...
def test_progress_line_shows_identity_and_budget() -> None:
    ...
    assert "[  3/20]" in line
    assert "007bbfb7" in line and "intervention" in line
...
def test_summary_table_has_one_row_per_condition() -> None:
    table: str = report.summary_table(
        [
            summarize([record("a", "baseline", True), record("b", "baseline", False)]),
            summarize([record("a", "intervention", True), record("b", "intervention", True)]),
        ]
    )
...
def test_comparison_block_reports_the_contingency_and_p_value() -> None:
    ...
    assert "baseline only" in block and "intervention only" in block
...
def test_task_table_adapts_to_the_number_of_conditions() -> None:
    single: str = report.task_table({"baseline": [record("a", "baseline", True)]})
    assert single.splitlines()[0].split() == ["Task", "baseline"]
    ...
    assert paired.splitlines()[0].split() == ["Task", "baseline", "intervention"]
    assert "4c/4i" in paired


def test_task_table_marks_missing_results() -> None:
    table: str = report.task_table(
        {"baseline": [record("a", "baseline", True)], "intervention": []}
    )
    assert table.splitlines()[-1].endswith("-")


def test_task_table_truncates_long_runs() -> None:
    many = [record(f"t{i:03d}", "baseline", i % 2 == 0) for i in range(70)]
    table: str = report.task_table({"baseline": many}, limit=10)
...
def test_full_report_includes_comparison_only_for_two_conditions() -> None:
    ...
    single: str = report.full_report({"baseline": [record("a", "baseline", True)]})
    assert "Paired comparison" not in single


def test_full_report_without_records() -> None:
    assert report.full_report({"baseline": [], "intervention": []}) == "No results recorded."
