...
def generator_self_debug(result: RunResult) -> str:
    """Baseline follow-up: execution feedback only, no external critic."""
    return (
        f"{execution_report(result)}\n\n"
        "Revise your rule and your program. Answer again with both sections "
        f"({RULE_HEADER} and {CODE_HEADER})."
    )
